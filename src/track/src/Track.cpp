...
Track::Track(SerialPort &sp, const char *mode) : sp_(sp), mode_(mode) {
    std::cout << "Track constructor: mode=" << (mode ? mode : "(null)") << std::endl;
    ...
    // ensure window is placed in a likely visible area (avoid moving off-screen)
    cv::moveWindow("track", 100, 100);
    cv::resizeWindow("track", cv::Size(300, 768));
    // start the window thread to improve GUI responsiveness when called from another thread
    cv::startWindowThread();
}


void Track::track(LfStack<ImageInfo> &image_info_stack) {
    std::cout << "Track::track() started" << std::endl;
...
void Track::updateVisionData() {
    // debug: confirm updateVisionData is being called and how many detected lights
    std::cout << "updateVisionData() called, cache lights=" << this->cache_.lights.size() << std::endl;
    cv::Point2d aim;
